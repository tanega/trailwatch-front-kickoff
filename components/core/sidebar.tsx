  ...
  return (
    <>
      <div className="fixed left-0 top-0 z-20 h-screen w-16 bg-[#3E4A68]">
        <div className="p-y-6e flex h-full flex-col items-center justify-start">
          <Link target="_blank" rel="noreferrer" href="/">
            <Icons.burger className="my-3 size-6 text-red-500" />
          </Link>
          <MenuIcon onClick={() => handleElementSelection("eye")}>
            <Icons.eye className="my-3 size-6" />
          </MenuIcon>
          <MenuIcon onClick={() => handleElementSelection("vessel")}>
            <Icons.vessel className="my-3 size-6" />
          </MenuIcon>
